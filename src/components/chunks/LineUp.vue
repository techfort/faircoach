<template>
    <div v-for="[id, player] in players" :key="player.id">
      <span :id="id">{{ player.roles[0] }}
        <span
          class="rounded-full text-gray-500 bg-gray-200 font-semibold text-sm flex align-center cursor-pointer active:bg-gray-300 transition duration-300 ease w-max m-2">
          ...
          <span class="flex items-center px-3 py-2 whitespace-nowrap">
            <strong class="text-bold text-lg">{{ player.playerNumber }}</strong>&nbsp;{{ player.name }}&nbsp;
          </span>
